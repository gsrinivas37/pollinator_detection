""""
Contributed by Mark: https://github.com/msmccor100
...
def generate_classification_report(true_labels_fp, predicted_labels_fp):
    true_labels_dict = read_true_labels(true_labels_fp)
    predicted_labels_dict = read_predicted_labels(predicted_labels_fp)
    Y_true = []
    Y_pred = []
    for key in predicted_labels_dict:
        if key in true_labels_dict:
            Y_pred.append(shortname_to_classnum[predicted_labels_dict[key]])
            Y_true.append(shortname_to_classnum[true_labels_dict[key]])
        else:
            print(f'Not there {key}??')
    labels = list(shortname_to_classnum.values())
    target_names = list(shortname_to_classnum.keys())
    report = classification_report(Y_true, Y_pred, labels=labels, target_names=target_names)
    print(report)



def generate_video_classification_result(true_labels_fp, predicted_labels_fp, outdir='../plots'):
    true_labels_dict = read_true_labels(true_labels_fp)
    predicted_labels_dict = read_predicted_labels(predicted_labels_fp)
...
#     generate_video_classification_result(true_labels_fp, predicted_labels_fp, outdir)

root_dir = '/Users/gsrinivas37/work/git/cs7643/pollinator_detection/mark_results'
generate_classification_report(root_dir+ '/true_classifications.csv', root_dir+'/predicted.csv')